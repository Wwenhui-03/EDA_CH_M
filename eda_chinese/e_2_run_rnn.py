...
def run_rnn(train_dir_save, base_dir ,train_path, test_path, categories,num_class,increment,see):
    ...
    return acc_test
def compute_baselines(writer,see):
...
def compute_aug(writer,see):
    ...
    for size_folder in size_folders:
        ...
        for i in range(len(dataset_folders)):
            ...
            word2vec_pickle = dataset_folder + '/word2vec.p'
            word2vec = load_pickle(word2vec_pickle)
            train_dir_save = dataset_folder + '/train_' + '_' + str(i)
...
if __name__ == "__main__":
    ...
    writer2 = open('aug_rnn/' + get_now_str() + '.csv', 'w')
    for i in range(0, 10):
        seed(i)
        print(i)
        compute_baselines(writer,i)
        compute_aug(writer2,i)
    writer.close()
    writer2.close()