...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        if flag:
            break
        acc_test=etest(config, model, test_iter)
        return acc_test
...
def etest(config, model, test_iter):
...
if __name__ == "__main__":
    ...
    filename = 'output_f2_bert/' + get_now_str()
    ...
    for see in range(5):
        ...
        for i, dataset_folder in enumerate(dataset_folders):
            ...
            dev_dir = os.path.join(dataset_folder, 'val.txt')
            category = categories[i]

            for increment in increments:                # calculate augmented accuracy
                print("AUG_Begin")
                config = Config('e2_aug',dataset, train_aug_st, dev_dir, test_path, category, see)
                start_time = time.time()
                print("Loading data...")
                train_data, dev_data, test_data = build_dataset(config, increment)  # 数据集预处理
                ...
                aug_acc = train(config, model, train_iter, dev_iter, test_iter)  # 开始训练
                ...
                config = Config('e2_orig',dataset, train_orig, dev_dir, test_path, category,see)
                start_time = time.time()
                print("Loading data...")
                train_data, dev_data, test_data = build_dataset(config, increment)  # 数据集预处理
                train_iter = build_iterator(train_data, config)
                ...
                model.to(config.device)
                orig_acc = train(config, model, train_iter, dev_iter, test_iter)
